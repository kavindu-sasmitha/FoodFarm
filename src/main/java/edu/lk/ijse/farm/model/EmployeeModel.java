...
public class EmployeeModel {
    public String saveEmployee(EmployeeDto employeeDto) throws SQLException, ClassNotFoundException {
        return CrudUtil.execute("INSERT INTO Employee (Employee_Id,Employee_Name,Contact,Join_Date,Email,Position) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    public String deleteEmployee(String employeeId) throws SQLException, ClassNotFoundException {
        return CrudUtil.execute("DELETE FROM Employee WHERE Employee_Id=?", employeeId) ? "Employee deleted successfully" : "Failed to delete employee";
    }

    public List<EmployeeDto> searchEmployee(String keyword) throws SQLException, ClassNotFoundException {
        ResultSet rst = CrudUtil.execute("SELECT * FROM Employee WHERE Employee_Id LIKE ? OR Employee_Name LIKE ? OR Contact LIKE ? OR Join_Date LIKE ? OR Email LIKE ? OR Position LIKE ?",
                "%" + keyword + "%",
                "%" + keyword + "%",
                "%" + keyword + "%",
                "%" + keyword + "%",
