...
public class LoginModel {
    public boolean logIn(String inputName, String inputPassword) throws SQLException, ClassNotFoundException {
        ResultSet rst = SQlUtil.execute("SELECT * FROM User WHERE User_Name = ? AND User_Password = ?", inputName, inputPassword);
        if (rst.next()) {
            return true;
        } else {
            return false;
    ...
    public boolean updateUserPassword(String userName, String newPassword) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            boolean result = SQlUtil.execute("UPDATE User SET User_Password=? WHERE User_Id=1", newPassword);
            connection.commit();
            return result;
